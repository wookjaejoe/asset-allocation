...
- **모멘텀 스코어 계산:** (최근 1개월 수익률 × 12) + (최근 3개월 수익률 × 4) + (최근 6개월 수익률 × 3) + (최근 12개월 수익률 × 1)- 4개 자산 모두 모멘텀 스코어 > 0이면 → 공격자산 유지
...
def run():
    ...
    def select_defensive(row: pd.Series):
        """
        ...
        ma = month_chart[Assets.defensive].iloc[pos - 11:pos + 1].mean()  # 최근 12개월 이동 평균

        # 12개월 수익률
        return ["BIL" if current[name] < ma[name] else name for name, _ in
                rate_of_return(current, 12).nlargest(3).items()]

    mmt = month_chart[list(set(Assets.canaria + Assets.aggressive))].apply(momentum_score, axis=1).dropna()
    aggressive_signal = mmt[Assets.canaria].ge(0).all(axis=1)

    tickers = pd.concat([
        mmt[aggressive_signal][Assets.aggressive].apply(lambda row: row.nlargest(6).index.tolist(), axis=1),
        month_chart.reindex(aggressive_signal.index)[~aggressive_signal].apply(select_defensive, axis=1),
    ]).sort_index().fillna(np.nan).reindex(chart.index, method="ffill").shift(1).dropna()

    make_report(chart, tickers, "./output/baa.html")
